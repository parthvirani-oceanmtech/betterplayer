...
class BetterPlayerController {
  ...
  Future setupDataSource(BetterPlayerDataSource betterPlayerDataSource) async {
    ...
    ///
    stopwatch.start();
    print("StopWatch during data source setup : ${stopwatch.elapsedMilliseconds}");
    ...
    Map<String, BetterPlayerAsmsTrack> resolutions = await getAllResolution(dataSource: betterPlayerDataSource);

    if (kDebugMode) {
      print("resolutionsresolutions" + resolutions.toString());
    }

    if (resolutions.length > 1) {
      setTrack(resolutions.entries.elementAt(1).value);
    } else {
      setTrack(BetterPlayerAsmsTrack.defaultTrack());
    }
  }

  Map<String, BetterPlayerAsmsTrack> getAllResolution({required BetterPlayerDataSource dataSource}) {
    // HLS / DASH
    final List<String> asmsTrackNames = dataSource.asmsTrackNames ?? [];
    final List<BetterPlayerAsmsTrack> asmsTracks = betterPlayerAsmsTracks;
    final Map<String, BetterPlayerAsmsTrack> resolution = {};
    for (var index = 0; index < asmsTracks.length; index++) {
      ...
      if (track.height == 0 && track.width == 0 && track.bitrate == 0) {
        preferredName = translations.qualityAuto;
      } else {
        preferredName = asmsTrackNames.length > index ? asmsTrackNames[index] : null;
      }
      resolution.addAll({(preferredName ?? '$index'): asmsTracks[index]});
    }

    if (resolution.isEmpty) {
      resolution.addAll({translations.qualityAuto: BetterPlayerAsmsTrack.defaultTrack()});
    }

    resolution.entries.toList().sort(((a, b) => (a.value.width ?? 0).compareTo((b.value.width ?? 0))));

    var sortedEntries = resolution.entries.toList()
      ..sort((a, b) => (a.value.width ?? 0).compareTo((b.value.width ?? 0)));

    // Clear the original map and reassign sorted entries
    resolution
      ..clear()
  ...
  Future _setupAsmsDataSource(BetterPlayerDataSource source) async {
    ...
    stopwatch.stop();
    print("stopwatch.elapsedMilliseconds.toString()" + stopwatch.elapsedMilliseconds.toString());
    ...
    if (data != null) {
      ...
      //     _betterPlayerAsmsAudioTracks!.forEach(
      //       (element) => print("Audio Tracks " + element.toString()),
      //     );
  ...
  void setTrack(BetterPlayerAsmsTrack track) {
    ...
    print("resolutions Current Playing Bitrate ${track.bitrate}");
